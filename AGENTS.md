...
22. `docs/superpowers/specs/2026-07-16-horizontal-only-page-vertical-scroll-target-design.md`（涉及 horizontal-only 页面、`anchorPagerScrollView` 纵向目标语义、默认 lookup opt-out、横向业务手势带动主容器或 Example 第五页时）
...
20. `docs/superpowers/plans/2026-07-16-horizontal-only-page-vertical-scroll-target.md`（执行、继续、审查或验收 horizontal-only 页面 nil 纵向 target、默认 lookup opt-out、横向区域 container 稳定性或相关文档契约时）
...
11. 2026-07-16 v0.7 最终生产代码 HEAD 为 `07a3443`：Host active/latest selection、Adapter matching terminal/Pageboy surface、统一 Interaction drain、系统返回优先级与双向跨 owner 惯性均已收口。fresh-pass 终态 Critical 0、Important 0、Minor 0；Framework 426/426、Example 60/60（16 单元 + 44 UI）与 generic Simulator build 全部通过。后续用户验收发现 Task 12 横向-only Example 页面错误把业务横向 `UIScrollView` 登记为纵向 target，导致横向拖动可以带动主容器；根因修复设计与计划已确认，完成 RED/GREEN、真实 UI、全量门禁和复审前，v0.7 Ready 暂停，不进入 v0.8。
...
v0.1–v0.6 及 v0.7 Task 0–15 已完成原验收；当前先执行 2026-07-16 横向-only 页面纵向目标回归专项，完成前 v0.7 Ready 暂停。不得提前进入 v0.8，也不得跳过 TDD、真实 UI、全量门禁或复审。
