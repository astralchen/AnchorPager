...
5. 2026-07-14 后续真实启动日志证明 `dfabd6c` 只修复了正式测量前的 zero-height layout，真实 Header 在 bootstrap 之前附着时仍会与旧 host `height == 0` 冲突。安装前 seed 修复设计已确认，书面规格、实施计划、RED/GREEN、全量验收和复审完成前，v0.5 Task 7 与 v0.6 Ready 再次关闭；不得进入 v0.7。
...
v0.1–v0.6 已完成；2026-07-14 plain bottom/bar 安全区与 Header bootstrap 测量专项实现、全量验收和整分支复审均已完成，v0.5 Task 7 与 v0.6 当前为 Ready。后续开发按版本路线进入 v0.7。
