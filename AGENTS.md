...
4. 2026-07-14 用户验收发现 plain page 底部 container 回弹把共享 viewport 整体上移，导致 bar 越过顶部安全区；同时首次 automatic Header 以 required `height == 0` 建立中立布局会触发示例内容约束冲突。专项修复设计已确认，实施、全量验收和独立复审待完成；因此第 3 条 Ready 只保留为历史结论，当前重新关闭 v0.5 Task 7 与 v0.6 Ready 门禁。
...
v0.1–v0.4 已完成；v0.5/v0.6 的 2026-07-13 Ready 保留为历史结论，但 2026-07-14 plain bottom/bar 安全区与 Header bootstrap 测量回归已重新打开修复门禁。必须先完成专项实现、全量验收和独立复审，再恢复 Ready；不得提前进入 v0.7。
