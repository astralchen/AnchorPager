...
6. 2026-07-14 用户进一步确认 `.insideSafeArea` 必须由主容器真实 `contentInset.top == safeArea.top` 表达、`.extendsUnderTopSafeArea` 的主容器 top inset 为 `0`，且业务 Header 在正常折叠中保持完整高度，由内部 presentation surface 上移并裁剪。专项设计已确认，详细实施计划已编写并完成计划自审；用户复核计划、TDD、全量验收和独立复审待完成。因此第 5 条 Ready 只保留为历史结论，当前重新关闭 v0.5 Task 7 与 v0.6 Ready 门禁，不得进入 v0.7。
...
v0.1–v0.6 的历史实现已完成，但 2026-07-14 主容器顶部 inset 与固定高度 Header presentation 专项已重新打开 v0.5 Task 7/v0.6 门禁。必须先完成设计对应的实施计划、TDD、全量验收和独立复审，再恢复 Ready；不得提前进入 v0.7。
