...
5. 2026-07-14 后续真实启动日志证明 `dfabd6c` 只修复了正式测量前的 zero-height layout，真实 Header 在 bootstrap 之前附着时仍会与旧 host `height == 0` 冲突。安装前 seed 修复书面规格与实施计划已确认；RED/GREEN、全量验收和复审完成前，v0.5 Task 7 与 v0.6 Ready 再次关闭，不得进入 v0.7。
...
v0.1–v0.4 已完成；v0.5/v0.6 的 2026-07-14 Ready 是历史结论。真实启动发现 Header 内容附着前仍存在 required zero-height 时序缺口，当前必须先完成安装前 seed 专项 RED/GREEN、全量验收和整分支复审；门禁恢复前不得进入 v0.7。
