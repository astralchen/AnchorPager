...
20. `docs/superpowers/specs/2026-07-14-default-extends-under-top-safe-area-design.md`（涉及 Header 顶部行为默认值、默认主容器 top inset、Example 初始模式或默认配置兼容性时）
21. `docs/superpowers/specs/2026-07-15-example-header-bounce-stable-content-layout-design.md`（涉及 Example Header 在 container 顶部回弹期间的内部文字位置、安全区约束或真实手势几何探针时）
22. `docs/superpowers/specs/2026-07-15-v0-7-interaction-selection-momentum-design.md`（涉及 v0.7 interaction state、选择队列、Tabman bar、Pageboy pan、跨 owner 惯性、reload/layout/尺寸仲裁或系统返回/业务横向手势优先级时）
23. `docs/superpowers/specs/2026-07-16-horizontal-only-page-vertical-scroll-target-design.md`（涉及 horizontal-only 页面、`anchorPagerScrollView` 纵向目标语义、默认 lookup opt-out、横向业务手势带动主容器或 Example 第五页时）
24. `docs/superpowers/specs/2026-07-16-compositional-layout-mixed-axis-gesture-validation-design.md`（涉及 `UICollectionViewCompositionalLayout` 混合轴页面、orthogonal section、纵向根 target、Pageboy 横向竞争或 Example 真实手势验证时）
25. `docs/superpowers/specs/2026-07-16-next-gesture-horizontal-boundary-paging-design.md`（涉及横向业务 scroll 自动边界判断、下一次手势接力 Pageboy、route gate、旧逐页静态分页开关移除或原生 orthogonal 边缘分页时）
26. `docs/superpowers/specs/2026-07-17-self-hosted-horizontal-paging-auto-handoff-design.md`（涉及替换 Tabman/Pageboy、AnchorPager 自有横向分页 Container/PagingScrollView、业务边缘下一手势自动接力、动态 recognizer priority、分段栏/indicator、页面 containment 或最近退场缓存时）
...
18. `docs/superpowers/plans/2026-07-14-default-extends-under-top-safe-area.md`（执行、继续、审查或验收 Header 顶部行为默认值、Example 初始顶部模式、默认配置兼容性或相关文档迁移时）
19. `docs/superpowers/plans/2026-07-15-example-header-bounce-stable-content-layout.md`（执行、继续、审查或验收 Example Header 在 container 顶部回弹期间的文字局部位置、约束关系或真实手势几何探针时）
20. `docs/superpowers/plans/2026-07-15-v0-7-interaction-selection-momentum.md`（执行、继续、审查或验收 v0.7 selection transaction、interaction state、Pageboy paging surface、跨 owner 惯性、reload/layout/尺寸仲裁或手势优先级时）
21. `docs/superpowers/plans/2026-07-16-horizontal-only-page-vertical-scroll-target.md`（执行、继续、审查或验收 horizontal-only 页面 nil 纵向 target、默认 lookup opt-out、横向区域 container 稳定性或相关文档契约时）
22. `docs/superpowers/plans/2026-07-16-compositional-layout-mixed-axis-gesture-validation.md`（执行、继续、审查或验收组合布局第六页、纵向 root target、orthogonal section、Pageboy 横向竞争、reload/lifecycle 或真实手势门禁时）
23. `docs/superpowers/plans/2026-07-16-next-gesture-horizontal-boundary-paging.md`（执行、继续、审查或验收横向业务边缘下一手势接力、route gate、旧逐页静态分页策略移除、原生 orthogonal 边缘分页或真实 UIKit 停止门禁时）
24. `docs/superpowers/plans/2026-07-17-self-hosted-horizontal-paging-auto-handoff.md`（执行、继续、审查或验收自有 PagingContainer/PagingScrollView、普通横向 scroll 与原生 orthogonal 第一任务硬门禁、分段栏/indicator、recentlyRetired、Host 迁移、static Bool 或 Tabman/Pageboy 删除时）
...
6. 2026-07-14 主容器 top inset 与固定高度 Header 专项最终生产 HEAD 为 `424a0a3`：`.insideSafeArea` 使用本地顶部遮挡作为真实 `contentInset.top`，`.extendsUnderTopSafeArea` 为 `0`；raw/logical offset、`H + D - I` range、固定 Header canonical presentation、container top 与 plain bottom 分层均已收口。fresh-pass 覆盖 `7885d9e...424a0a3`，发现的 2 个 Important（safe-area/bounds 变化在 `I/D` 不变时未取消 active boundary、缺少 `willSelect` 的 plain selection completion 未独立清理 page surface）和 2 个 Minor（inset-only geometry 迁移重复边界日志、Public DocC 仍描述缩高）已分别在 `5ba84d4`、`424a0a3` 以 RED/GREEN 修复，终态为 Critical 0、Important 0、Minor 0。Apple Swift 6.3.3 / Xcode 26.6 下，Framework 322/322、Example 41/41（11 单元 + 30 UI）与 generic Simulator build 全部通过，0 fail、0 skip、0 error/warning/analyzer warning；v0.5 Task 7 与 v0.6 恢复 Ready，可进入 v0.7。
7. 2026-07-15 Header 默认顶部行为专项最终生产代码 HEAD 为 `3bdcfb6`：`AnchorPagerHeaderConfiguration.init` 的唯一默认参数改为 `.extendsUnderTopSafeArea`，显式 `.insideSafeArea`、主容器 raw/logical offset、固定 Header、viewport 裁剪、bar、Pageboy containment 和业务 child ownership 均未改变。Apple Swift 6.3.3 / Xcode 26.6 下，Framework 322/322、Example 41/41（11 单元 + 30 UI）与 generic Simulator build 全部通过，0 fail、0 skip；三份 xcresult 均为 0 error、0 warning、0 analyzer warning，运行时约束关键字零命中。fresh-pass 覆盖 `97e8fc2...f4d9f41`，终态 Critical 0、Important 0、Minor 0；v0.5 Task 7 与 v0.6 继续为 Ready，可进入 v0.7。
8. 2026-07-15 Example Header 顶部回弹内容稳定专项生产代码 HEAD 为 `1f7e3f4`：标题栈采用顶部安全下限与底部稳定等式，真实 container 回弹跨越完整顶部遮挡时 Header 高度与文字相对 Header 顶部距离都保持固定；AnchorPager framework、Header 高度模型、container presentation、Pageboy containment 和业务 child ownership 均未改变。旧约束隔离 RED 的真实 UI 最大内容漂移为 `116 pt`，正式约束聚焦 GREEN 通过。Apple Swift 6.3.3 / Xcode 26.6 下，Framework 322/322、Example 41/41（11 单元 + 30 UI）与 generic Simulator build 全部通过，0 fail、0 skip；三份 xcresult 均为 0 error、0 warning、0 analyzer warning，UIKit `LayoutConstraints` 查询无冲突。fresh-pass 覆盖 `afaefce...1f7e3f4`，终态 Critical 0、Important 0、Minor 0；v0.5 Task 7 与 v0.6 继续为 Ready，可进入 v0.7。
9. 2026-07-15 v0.7 源码审查与专项设计已确认：PagingHost 将成为唯一 active/latest pending selection request owner，Adapter 只执行 Tabman/Pageboy 并观察 paging surface，InteractionCoordinator 只做跨域状态仲裁，Scroll/Overscroll 保持唯一 offset/policy owner；双向跨 owner velocity、reload/layout/尺寸仲裁和系统返回/业务横向优先级均已纳入设计，但生产实现、TDD、UI 验收和 fresh-pass 尚未开始，v0.7 仍为 Not Started。
10. 2026-07-16 v0.7 Task 0–14 已在当前开发分支完成任务级 TDD、自审与真实 UI 验收：统一 selection/interaction、Pageboy surface、双向跨 owner 惯性和系统返回优先级已落地；Task 14 为 Framework 187/187、Example unit 16/16、新增 UI 6/6、v0.5/v0.6 边界回归 15/15，均 0 fail、0 skip。真实 UIKit 已否定任意业务横向 child 自动优先方案，生产撤回 direct child relation 与全部 guard，当前限制已同步到长期文档。Task 15 全量 Framework/Example/build、warning/analyzer/runtime constraint 与整分支 fresh-pass 尚未完成，v0.7 暂不标记 Ready。
11. 2026-07-16 v0.7 原最终生产代码 HEAD 为 `07a3443`：Host active/latest selection、Adapter matching terminal/Pageboy surface、统一 Interaction drain、系统返回优先级与双向跨 owner 惯性均已收口。fresh-pass 终态 Critical 0、Important 0、Minor 0；Framework 426/426、Example 60/60（16 单元 + 44 UI）与 generic Simulator build 全部通过。后续用户验收发现 Task 12 横向-only Example 页面错误把业务横向 `UIScrollView` 登记为纵向 target，导致横向拖动可以带动主容器；该缺口的最终修复与恢复门禁见下一项。
12. 2026-07-16 横向-only 页面纵向目标修复生产代码 HEAD 为 `984a009`：Example 第五页关闭默认 lookup，不再把业务横向 `UIScrollView` 登记为 `anchorPagerScrollView`，Store 提交 original Pageboy page + nil 纵向 target；Framework ScrollCoordinator、ContainerScrollView、Paging adapter、containment/lifecycle 与业务 delegate/pan/bounce ownership 均未改变。RED 精确命中旧目标声明；GREEN 后 Example unit 16/16、新真实横向手势 UI 1/1、相邻 UI 3/3 通过。Apple Swift 6.3.3 / Xcode 26.6 下，Framework 426/426、Example 61/61（16 单元 + 45 UI）与 generic Simulator build 全部通过，0 fail、0 skip、0 error/warning/analyzer warning；运行时问题关键字零命中。fresh-pass 首轮 Critical 0、Important 0、Minor 2，两处文档 Minor 已修正，终态 Critical 0、Important 0、Minor 0；v0.7 恢复 Ready，可进入 v0.8。
13. 2026-07-16 Compositional Layout 混合轴专项生产代码 HEAD 为 `db4b9bc`：Public DataSource 新增默认 `true` 的逐页 `allowsInteractiveHorizontalPagingAt`，策略随 reload metadata 原子提交，由 PagingHost 独占 committed snapshot，Adapter 只执行 Pageboy 自有 `isScrollEnabled`；Example 第六页以根 `UICollectionView` 为唯一纵向 target，orthogonal section 表达横向业务滚动，index 4、index 5 显式 `false`，业务横向内容到边界后不向 Pageboy 接力，bar/API 保持可用。追加第六页暴露的旧第五页末端边界依赖已通过二次设计修订、RED/GREEN 与 index 3→4 真实 terminal 收口。Apple Swift 6.3.3 / Xcode 26.6 下，Framework 439/439、Example 70/70（19 单元 + 51 UI）与 generic Simulator build 全部通过，0 fail、0 skip、0 error/warning/analyzer warning；运行时问题关键字零命中。fresh-pass 覆盖 `0db297d...db4b9bc`，终态 Critical 0、Important 0、Minor 0；v0.7 继续为 Ready，可进入 v0.8。
14. 2026-07-16 横向业务边缘下一手势分页接力设计已确认：后续目标是删除逐页 `allowsInteractiveHorizontalPagingAt` 及 generation-aware 策略链，不增加外部路由 API；框架以自有 route gate 在手势起点沿命中路径读取公开 `UIScrollView` 边界，内容仍可消费则本次归业务，已在对应边缘则让 Pageboy 原生分页，同一次手势中途到边缘不接力。必须保留 Compositional Layout 原生 orthogonal，不设置 Pageboy/业务内建 pan delegate、不写业务 offset、不识别私有类名；普通嵌套 scroll 与原生 orthogonal 的真实 UIKit 硬门禁、TDD、实现、完整验收和 fresh-pass 均尚未开始，当前生产仍使用上一项静态策略。
15. 2026-07-16 横向业务边缘下一手势 route gate 真实 UIKit 停止门禁已执行并失败：Task 1/2 曾完成纯边界模型与独立 gate，Task 3 临时装配 Framework 51/51 通过，但普通业务横向 scroll 与原生 orthogonal 的 UI 为 0/2，两类内容首笔 interior 拖动都未消费，失败证据为 `/private/tmp/AnchorPagerTask3UIKitGate-20260716-1840.xcresult`。全部未提交 Task 3 装配、测试和 Example 临时变更已用 `apply_patch` 清理；清理前 Task 1/2 聚焦回归为 21/21。用户随后确认删除未装配的 Task 1/2 实验源码、专属测试和日志断言，失败证据与历史设计记录继续保留；清理后 Framework 439/439 与 Example generic Simulator build 通过。自动 route-gate 方案在当前公开 UIKit 与 delegate/pan/offset/enable/bounce ownership 约束下被否定；生产继续使用 `allowsInteractiveHorizontalPagingAt` 静态链，Task 4 及后续删除迁移、完整验收和 fresh-pass 均为 Blocked/Not Started，未经新设计不得继续。
16. 2026-07-17 用户已确认以自有分页架构取代上一项被真实 UIKit 0/2 门禁否定的 Pageboy 外挂 route-gate：最终目标是由 `AnchorPagerPagingContainerViewController` 管理页面 containment，由 `AnchorPagerPagingScrollView` 只管理分页物理，AnchorPager 自行实现分段栏/indicator，并在新链路真实 UIKit 硬门禁通过后删除 Tabman/Pageboy 和 `allowsInteractiveHorizontalPagingAt`。手势采用自有 route recognizer 与自有 paging pan 共享单次决策，当前命中业务 pan 仅在本次触摸动态等待 route；同手势不接力、下一手势自动分页。消失页 terminal 当场解除 containment，但实例进入有界 `recentlyRetired` 单槽。正式设计已确认，实施计划、TDD、真实 UI、全量验收和 fresh-pass 均尚未开始；门禁通过前当前静态生产链保持不变。
17. 2026-07-17 自有横向分页详细实施计划已完成并自审，共 9 个任务；Task 1 已按真实 Framework 临时执行模式实施，首轮 4 条真实 UIKit 停止门禁为 2/4：普通横向 `UIScrollView` 与普通区域/page boundary bounce 通过，原生 orthogonal 回到起点后的下一手势未分页，末页业务 bounce 探针未达到。失败证据为 `/private/tmp/AnchorPagerSelfHostedTask1UIKitGate-1.xcresult`；实验生产、测试、Host/Example 开关与探针已用 `apply_patch` 完整清理，Framework 439/439 与 Example generic Simulator build 通过，当前 Tabman/Pageboy、Adapter 和 static Bool 生产链保持不变。Task 2–9 为 Blocked，重启前必须先修订阶段 0 设计，不得从已清理实验继续实现。

> **临时技术基线例外：** 上述第 16 项设计的实施明确获准在真实 UIKit 停止门禁通过后替换本文件「技术基线」第 9–11 项以及「架构边界」中 Tabman/Pageboy 执行层要求。门禁通过和 Host 执行层迁移前，当前 Tabman/Pageboy 生产事实继续有效；不得在隔离原型尚未证明普通横向 scroll 与原生 orthogonal 可行前提前删除依赖或 Public Bool。
...
v0.1–v0.6、v0.7 Task 0–15、2026-07-16 横向-only 页面纵向目标回归及 Compositional Layout 页面级分页专项均已完成 TDD、真实 UI、全量门禁和 fresh-pass；v0.7 当前为 Ready，可进入 v0.8。后续版本仍不得跳过对应设计、计划、TDD、UI 验收或复审门禁。
