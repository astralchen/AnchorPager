...
1. v0.5 纵向 handoff、plain direct page、stable/native boundary 分离和 v0.6 三种顶部 mode 的初始实现到 `47abcd6`；初次独立复审的 3 个 Important 已修复到 `f81ca1e`，再次整分支复审剩余的零稳定区间边界反向切换 Important 已修复到 `5b80893`，对应架构文档 Minor 同步修正。
2. 2026-07-13 最新修复新鲜验收为 Framework 276 项、Example 37 项（10 单元 + 27 UI）、0 fail、0 skip，generic Simulator build 成功；三份 xcresult 均为 0 error、0 warning、0 analyzer warning。
3. 修复后的再次独立复审待执行；复审应覆盖 `be2d783...HEAD`，并重点比较修复段 `b00d204...5b80893`。复审完成前不得把 v0.5/v0.6 标记 Ready，也不得声称 Critical/Important 已由独立审查清零。
