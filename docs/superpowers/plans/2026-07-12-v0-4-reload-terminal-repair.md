...
- [ ] **Step 5: selection 中 reload 使用 latest pending request**

Adapter 提供无副作用 reload-readiness；Host 在 readiness false 时暂存最新 titles/count/index，不调用 Pageboy。
did/cancel terminal 到达后，若有 pending reload，则不向上层提交旧 selection，立即执行最新 request。覆盖 animated
selection→empty、interactive cancel→empty、selection→连续 empty/nonempty latest-wins，并断言没有 assertion、
假 reload terminal 或 pending generation 提前提交。pending 期间 `setSelectedIndex` 返回 false。

- [ ] **Step 6: 把根布局 containment 改为稳定 Host**
