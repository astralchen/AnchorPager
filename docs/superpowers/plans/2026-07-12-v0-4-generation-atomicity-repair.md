...
- [x] **Step 1: 写 request identity RED 测试**
...
- [x] **Step 3: 实现 pending/active request 状态**
...
- [x] **Step 4: terminal 后推进 latest pending**
...
- [x] **Step 1: 写 provider/visible generation RED 测试**
...
- [x] **Step 2: 写 migration 可变状态隔离 RED 测试**
...
- [x] **Step 4: 拆分 live payload 与 generation state**
...
- [x] **Step 5: 分离 provider/visible/committed generation**
...
- [x] **Step 6: 让 ownership release 发生在 commit 边界**
...
- [x] **Step 7: 运行 Store + inset/fallback 回归**
...
- [x] **Step 8: 提交 generation state 隔离**
...
- [x] **Step 3: 运行 ViewController 测试并确认 RED**
...
- [x] **Step 4: 实现 ReloadSnapshot staging**
...
- [x] **Step 5: 实现 willPerform provider activation**
...
- [x] **Step 7: 更新 pre-load selection 与 viewDidLoad submit**
...
- [x] **Step 8: 删除无 request identifier 临时兼容桥**
...
实际结果（2026-07-12，严格串行复用同一 iPhone 17）：

- Apple Swift 6.2.4；`git diff --check` 通过；`swift package resolve` 1.34 秒通过。
- Framework 193 项，0 fail、0 skip；xcresult 区间约 45.12 秒，墙钟 53.81 秒。
