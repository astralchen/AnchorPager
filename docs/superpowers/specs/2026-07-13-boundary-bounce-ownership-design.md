...
**状态：** 已完成；2026-07-13 历史实现与复审、2026-07-14 plain bottom 页面/chrome presentation 修订、完整重新验收和整分支 fresh-pass 复审均已完成，v0.5/v0.6 当前为 Ready
...
container owner 的可见 overflow 物理：
...
2. 无滚动页 container 底部 owner 仍使用 `verticalScrollView` 原生物理，但只移动 Paging adapter 内 Pageboy 页面 surface；Header/bar 保持 canonical 吸顶。
...
5. `AnchorPagerLayoutContext` 继续表示实际可见坐标：container top 三个 frame 同移，plain bottom 只有 content frame 上移。
...
2. 无滚动页 container 底部 overflow 只使 Pageboy 页面 content frame/surface 同步上移，Header/bar 保持 canonical，contentSize/range 不变。
...
1. 先提取通用 boundary pass-through 与当时的 container presentation，完成历史 v0.5 Task 7；plain bottom 的 presentation target 已由 2026-07-14 专项设计进一步修订。
...
- 实现者十项自审未发现阻塞性代码缺陷；该初始验收随后进入独立复审并发现下述 3 个 Important。该句保留历史过程，最终 Ready 结论以本节末的第四次整分支独立复审与最终验收记录为准。
...
随后第二次整分支复审发现零稳定区间可以从一侧 overflow 直接跳到相反侧、不会经过 stable callback；旧 `begin` 会无条件返回旧 active owner。`5b80893` 将“不同 boundary 且旧 owner 未呈现时同步 finish 并继续新 route”收口在纯 Overscroll policy，同 boundary 和已呈现 owner 语义保持不变；architecture 同步补齐 top/bottom 对称公式与当前 v0.5/v0.7 职责。

第三次整分支复审发现：已呈现 `.top/.child` owner 从 child raw overflow `-12` 越过到 `+6` 时，observer finish 递归执行通用 stable settle，把 raw total 6 错误扩张到 collapsed container + child 6；KVO/pan callback 顺序因此会影响结果。`128821f` 让 enforcement 只返回 finish result/owner：带 pan input 时同轮应用当前 Resolver 输入；observer-only `.top/.child` 则保存 `containerBoundary + rawChildDistance`，经同一 Resolver container-first 分配到 `container = 6, child = top`。其他 top/container、child bottom、container bottom finish 保持原语义，业务 child bounce 配置不变。同期 requirements 的 guarded apply/skip 逐帧日志旧条款已改为 owner/handoff/boundary 状态变化或受控采样。

第四次整分支独立复审覆盖 `be2d783...13b3d95`，结论为 Critical 0、Important 0、Minor 2。README 旧验收摘要已更新；`testRealChildContainerTopBounceIsVisible` 已增加严格 `XCTAssertLessThan(state.childTopMax, 0.5)`，与 container 可见 presentation 断言共同证明 `.container` 顶部 owner 排他。历史验收为 Framework 283/283、Example 37/37 与 generic Simulator build 全部通过。

2026-07-14 后续用户验收发现本规格的“plain bottom 整个 viewport 上移”会把 bar 推过顶部安全区；真实 child bottom 因 child owner 保持正常。用户确认：`verticalScrollView` 继续提供 plain bottom 原生物理，但只有 Pageboy 页面内容区域产生向上 presentation。该修订以 `2026-07-14-plain-bottom-page-presentation-header-bootstrap-measurement-design.md` 为最新契约；实现、完整验收和整分支 fresh-pass 复审已在生产代码 HEAD `c37e829` 完成，本规格关联的 v0.5/v0.6 当前为 Ready。
