...
**状态：** 页面级横向分页策略方向已确认；修订规格已完成，待用户书面复核后重写实施计划并执行 Framework TDD
...
6. 组合布局页提交页面级策略，静态关闭该页的 Pageboy 横向拖拽分页；Tabman bar 与公开 `setSelectedIndex(_:animated:)` 仍可离开该页。
...
组合布局 Example 只对 index 5 返回 `false`。结果是：

1. orthogonal section 成为该页唯一横向手势 owner，左右双向拖拽都由 UIKit 业务内容消费；
2. 普通纵向 cell 区域不再通过横向拖拽离开该页；
3. 用户仍可点击分段栏或调用公开 API 离开该页；
4. 从允许交互分页的相邻页仍可横向拖入该页，matching did-select 提交后才静态关闭目标页的后续分页拖拽。
...
4. 从默认允许交互分页的 index 4 仍可横向拖入 index 5；提交 index 5 后，后续页面横向拖拽才被关闭。
...
7. Example data source 对 index 5 返回 `false`，其他 index 继承默认 `true`。
...
4. 从 index 4 横向拖入 index 5 后再次拖动 orthogonal 区域，证明 target terminal 后已关闭 Pageboy pan。
5. 相邻 reload/rebind 回归：reload 或离场再返回后，index 5 仍提交根 CollectionView target和 `false` 策略，旧页面资源不再更新 probe。
...
已按本设计先建立 RED，再完成 Example-only 最小实现。当前证据为：

1. Example unit 18/18 通过；第六页、根 `UICollectionView` identity、纵向 range、业务 delegate/pan/bounce 配置和横向进度纯值语义均成立。
2. `testCompositionalVerticalRegionHandsOffToCollectionView` 1/1 通过；根 CollectionView 作为 committed 纵向 target 完成 container-to-child handoff。
3. `testCompositionalNonOrthogonalRegionStillPages` 1/1 通过；普通纵向 cell 内的横向 drag 仍由 Pageboy 返回 index 4。
4. `testCompositionalOrthogonalRegionOwnsHorizontalDrag` 在真实触点从 `(272.93, 356)` 移动到 `(72.40, 374)` 后稳定失败。该轨迹完整位于首张横向卡片 `(16, 275, 313.33, 180)` 内；拖动前后首、次卡片 frame 完全不变，`horizontalCurrent/horizontalMax` 均为 `0`，Header、container、child 纵向指标和 selection terminal 均为 `0`。
5. 因此失败不是 layout range、cell 命中、invalidation handler 漏记或纵向 owner 错误，而是 Pageboy paging pan 与 UIKit orthogonal 内部 pan 的同向竞争使业务 pan 未成为 winner；尾页会把 Pageboy 边界位移同步钳回，所以没有 selection terminal。

该结果与 v0.7 已固定的任意业务横向 scroll 限制一致。已验证并撤回的 `pagingPan -> childPan`、业务子树/共同祖先 hit-region guard、simultaneous/non-preventing guard 不能复用；Compositional Layout 又不通过公开 API 暴露 orthogonal 内部 scroll/pan identity，因此不能建立一个更窄且可撤销的静态 failure relation。

### 已确认修订边界

截至本门禁记录：

1. `Sources/AnchorPager` 仍为零改动，Public API、Pageboy containment、selection transaction、业务 delegate/pan/bounce ownership 均未改变。
...
4. 用户已确认新增**页面级横向分页策略**：当 committed page 返回 `false` 时，Adapter 在 matching terminal 边界静态关闭该页的 Pageboy 手势分页；Tabman bar 与公开 `setSelectedIndex` 仍可切页，并明确放弃“同页非正交区域仍可横滑分页”。

当前 Example 代码仍只是门禁阶段工作，不把 orthogonal winner 标记为已交付能力，也不提交为完成状态。本文完成书面复核后，必须先重写计划并从 Framework/Public RED 继续。
