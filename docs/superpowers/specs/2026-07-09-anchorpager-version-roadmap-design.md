...
设计草案：`2026-07-13-v0-5-scroll-coordination-design.md`，待用户审阅。v0.4 最终独立复审没有 Critical/Important/Minor 未解决项；v0.5 实现前仍须完成设计审阅和详细计划。ScrollCoordinator 只能只读 Store 已提交的 current child/scroll target（empty 为 nil），并在 matching reload/selection complete/cancel terminal 后重新绑定；任何时刻都不得设置业务 child 的 `UIScrollView.delegate`，也不得替换 child pan delegate、缓存 Host/adapter/provider、读取 provider pending、复制 page identity/cache/generation 职责，或绕过空态猜测 current owner。
