...
设计与计划基线：`2026-07-13-v0-5-scroll-coordination-design.md`、`2026-07-13-boundary-bounce-ownership-design.md`、`2026-07-14-container-top-inset-fixed-header-presentation-design.md` 及对应 plans。2026-07-14 主容器真实 top inset、逻辑 container offset 和固定高度 Header presentation 已完成 TDD 实现与首轮全量验收：Framework 318/318、Example 41/41、generic Simulator build，均为 0 fail/skip/error/warning/analyzer warning；实现 HEAD `1847aac`，验收 HEAD `ce09f2b`。最终 fresh-pass 尚未完成，因此继续关闭 v0.5 Task 7/v0.6 Ready 并阻止进入 v0.7。ScrollCoordinator 只能只读 Store 已提交的 current child/scroll target（empty 为 nil），并在 matching reload/selection complete/cancel terminal 后重新绑定；任何时刻都不得设置业务 child 的 `UIScrollView.delegate`，也不得设置 container/child 内建 pan delegate、缓存 Host/adapter/provider、读取 provider pending、复制 page identity/cache/generation 职责，或绕过空态猜测 current owner。
