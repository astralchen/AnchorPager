...
设计与计划基线：`2026-07-13-v0-5-scroll-coordination-design.md`、`2026-07-13-boundary-bounce-ownership-design.md`、`2026-07-14-container-top-inset-fixed-header-presentation-design.md` 及对应 plans。v0.5 Task 1–7 与 v0.6 曾完成历史验收，但 2026-07-14 用户确认主容器真实 top inset、逻辑 container offset 和固定高度 Header presentation 新契约；专项实施计划、TDD、全量验收和独立复审尚未完成，当前重新关闭 v0.5 Task 7/v0.6 Ready 并阻止进入 v0.7。ScrollCoordinator 只能只读 Store 已提交的 current child/scroll target（empty 为 nil），并在 matching reload/selection complete/cancel terminal 后重新绑定；任何时刻都不得设置业务 child 的 `UIScrollView.delegate`，也不得设置 container/child 内建 pan delegate、缓存 Host/adapter/provider、读取 provider pending、复制 page identity/cache/generation 职责，或绕过空态猜测 current owner。
