...
设计与计划基线：`2026-07-13-v0-5-scroll-coordination-design.md`、`../plans/2026-07-13-v0-5-scroll-coordination.md`。v0.4 最终独立复审没有 Critical/Important/Minor 未解决项；v0.5 实施中，Task 1 已完成，Task 2 已根据 UIKit 实测和 JXPagingView 参考验证完成设计修订。ScrollCoordinator 只能只读 Store 已提交的 current child/scroll target（empty 为 nil），并在 matching reload/selection complete/cancel terminal 后重新绑定；任何时刻都不得设置业务 child 的 `UIScrollView.delegate`，也不得设置 container/child 内建 pan delegate、缓存 Host/adapter/provider、读取 provider pending、复制 page identity/cache/generation 职责，或绕过空态猜测 current owner。
...
13. v0.5 完全展开后的额外下拉临时只允许 container 原生 bounce，child 保持顶部；正式 overscroll mode 仍属于 v0.6。
...
9. Example UI test 使用真实连续 drag 覆盖双向 handoff、短内容、fallback、切页和唯一 container bounce。
