...
设计与计划基线：`2026-07-13-v0-5-scroll-coordination-design.md`、`2026-07-13-boundary-bounce-ownership-design.md` 及对应 plans。v0.4 最终独立复审没有 Critical/Important/Minor 未解决项；v0.5 Task 1–6、plain direct page 与 stable/native boundary 已实现，第三次整分支复审问题已修复并通过 Framework 283 / Example 37 / generic build 新鲜验收，修复后的再次独立复审与 Ready 决策待执行。ScrollCoordinator 只能只读 Store 已提交的 current child/scroll target（empty 为 nil），并在 matching reload/selection complete/cancel terminal 后重新绑定；任何时刻都不得设置业务 child 的 `UIScrollView.delegate`，也不得设置 container/child 内建 pan delegate、缓存 Host/adapter/provider、读取 provider pending、复制 page identity/cache/generation 职责，或绕过空态猜测 current owner。
