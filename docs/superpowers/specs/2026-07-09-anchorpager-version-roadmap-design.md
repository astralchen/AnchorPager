...
设计与计划基线：`2026-07-13-v0-5-scroll-coordination-design.md`、`../plans/2026-07-13-v0-5-scroll-coordination.md`。v0.4 最终独立复审没有 Critical/Important/Minor 未解决项，v0.5 设计与计划门禁已满足但实现尚未开始。ScrollCoordinator 只能只读 Store 已提交的 current child/scroll target（empty 为 nil），并在 matching reload/selection complete/cancel terminal 后重新绑定；任何时刻都不得设置业务 child 的 `UIScrollView.delegate`，也不得替换 child pan delegate、缓存 Host/adapter/provider、读取 provider pending、复制 page identity/cache/generation 职责，或绕过空态猜测 current owner。
...
12. simultaneous recognition 只通过自有 container pan forwarding proxy 放行当前 committed child pair；child scroll/pan delegate 保持原 owner。
