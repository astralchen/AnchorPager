...
当前状态：主体实现、reload terminal 修复与完整验收已通过，待最终独立复审。修复后的稳定契约是 paging host、PageStateStore committed current child/scroll target、page/empty reload terminal 和 selection terminal；Pageboy 5.0.2 空态 teardown shim 是依赖升级门禁。
...
启动门禁：v0.4 最终独立复审没有 Critical/Important 未解决项后才能启动。v0.5 只能依赖稳定 paging host、Store 已提交的 current child/scroll target 和标准化 reload/selection terminal；不得缓存可替换 adapter、复制 page identity/cache/generation 职责，也不得绕过空态以猜测 current owner。
