...
> **2026-07-17 路线修订：** 本文的 Tabman/Pageboy 条目保留 v0.1–v0.7 的历史技术路线和已验收事实。用户已确认后续以 AnchorPager 自有 PagingContainer/PagingScrollView、分段栏、indicator 与 containment 取代第三方执行层；只有新真实 UIKit 停止门禁通过后才删除依赖和旧 Public Bool。正式设计见 `2026-07-17-self-hosted-horizontal-paging-auto-handoff-design.md`，实施计划与生产迁移尚未开始。
...
6. Swift 6 language mode 并发检查问题不通过 `@unchecked Sendable`、`nonisolated(unsafe)` 或 `@preconcurrency` 粗暴压制。
...
15. 修复问题或编写新功能前必须全面梳理数据流、状态所有权、约束或回调关系、相邻版本职责、回归路径和文档契约；未完成关系梳理不得开始实现代码。
16. 如果关系梳理发现现有设计或架构存在职责闭环、所有权冲突、跨层泄漏、状态语义矛盾或会阻碍后续版本扩展，必须立即停止局部实现并提醒用户，先修订设计、架构或计划文档，不得用局部补丁掩盖问题。

## 依赖基线

AnchorPager 的最低工具链为 Swift 6.2，语言模式为 Swift 6，最低系统版本为 iOS 14，包管理器为 Swift Package Manager。
`swift-tools-version: 6.2` 建立工具链门禁，`swiftLanguageModes: [.v6]` 只表示 Swift 6 language mode。
已完成版本中的“Swift 6”是当时的语言模式/执行记录，不机械改写；当前及后续开发统一使用 Swift 6.2+ 工具链。
...
AnchorPagerViewController 是唯一 public 容器入口。它负责数据加载、Header 承载、页面选择状态、外部配置、delegate 回调、page lifecycle 策略和对外状态语义。横向 page 的实际 UIKit containment 由内部 Tabman/Pageboy adapter 执行，AnchorPager 不对同一个 page view controller 重复 `addChild`。
...
5. `Children`：page state store、缓存窗口、offset snapshot 和 committed scroll identity；无滚动页保持 original controller 直接 Pageboy containment，不建立 fallback scroll host。
6. `Paging`：Tabman/Pageboy adapter，负责横向分页、分段栏渲染、indicator、切页事件收敛和横向 page containment 执行。
...
7. 实现 fallback host 基础 containment；横向 page 的实际 containment 由 Tabman/Pageboy adapter 执行。
...
10. 为 reloadData、Header 承载、fallback containment、分页切换、越界 no-op、fallback host 等 v0.1 关键事件加入日志。
...
当前默认已通过 2026-07-14 默认顶部行为专项调整为 `extendsUnderTopSafeArea`；`insideSafeArea` 仍可显式选择。该默认变化不修改两种模式的几何、纵向 owner、Pageboy containment 或业务 child 滚动所有权。

专项生产代码 HEAD `3bdcfb6` 已于 2026-07-15 完成最终验收：Framework 322/322、Example 41/41（11 单元 + 30 UI）与 generic Simulator build 全部通过，运行时约束关键字零命中，fresh-pass `97e8fc2...f4d9f41` 为 Critical 0、Important 0、Minor 0。v0.5 Task 7 与 v0.6 的 Ready 门禁不受默认值迁移影响。
...
详细架构以 `2026-07-11-fixed-paging-viewport-inset-scroll-ownership-design.md` 为准。
...
6. 支持关闭默认查找。
7. `anchorPagerScrollView` 只表示纵向协调目标；horizontal-only 页面关闭默认查找并提交 nil target，混合页面只登记纵向父 scroll。
8. 默认查找不使用 `contentSize`、bounce 或手势 velocity 推断轴向。
9. 无候选 UIScrollView 时保留 original Pageboy page + nil scroll target，不创建替代 scroll host。
10. `AnchorPagerBarConfiguration.height` 改为 optional，默认 nil 使用 Tabman 自适应高度，显式值约束实际 bar。
11. Tabman adapter 高度固定为 Header 完全折叠时的 viewport 高度，Header 折叠热路径只移动 adapter top。
12. 通过 Tabman public `barInsets.top` 获取 child 局部 bar obstruction，不把 Header 或容器顶部遮挡写入 child top inset。
13. 区分 managed inset 和外部追加 inset，不覆盖调用方原有 contentInset。
14. AnchorPager 接管的 child scroll view 设置 `contentInsetAdjustmentBehavior = .never`，ownership 结束时恢复原始行为。
15. 使用弱引用 ownership record，重复目标跳过写入并在释放时只移除最后一次 managed 部分。
16. 记录显式 scroll view 命中、默认 lookup 命中、nil scroll target、barInsets 变化、managed inset 写入、跳过和归还日志。

验收：

1. 显式 scroll view 优先测试通过。
2. 默认嵌套查找、过滤规则、多候选稳定性测试通过。
3. 关闭默认查找和无 scroll view direct page + nil target 测试通过。
4. 不跨 child view controller 边界测试通过。
5. managed inset 不覆盖外部 contentInset 测试通过。
6. Header 折叠期间 Tabman adapter height 和 Pageboy child bounds 保持不变。
7. optional bar height、自适应 barInsets、ownership 归还和 nil target 测试通过。
8. 示例工程真实列表页和 plain direct 页 UI test 通过。

### v0.4：Child 生命周期与缓存版

目标：让 Tabman 驱动的 page lifecycle 语义、reload、缓存和释放行为可信。

当前状态：主体实现、reload terminal 修复、generation atomicity Task 1–3 与 Swift 6.2.4 完整验收已完成，待最终独立复审。稳定契约是 Host request/terminal 串行与 bar acknowledgement、Store live identity payload/generation-specific lease/snapshot/CleanupPlan、ViewController staged public/provider/bar terminal 与 pre-load，以及严格 committed current child/scroll target；Pageboy 5.0.2 空态 teardown shim 是依赖升级门禁。最新验收为框架 193 项、Example 5 项单元与 16 项 UI 测试全部通过，0 fail、0 skip；resolve 1.34 秒、框架墙钟 53.81 秒、Example build 15.71 秒、Example 全量墙钟 277.97 秒；warning 仅为 Pageboy/Tabman privacy resource 提示。

范围：

1. 实现 page state store，并重定位或替换 v0.1 的 `AnchorPagerChildViewControllerStore`，避免与 Tabman/Pageboy 对同一个 page view controller 双重 containment。
2. 定义缓存窗口，默认至少保留 current page，可配置是否保留相邻页。
3. 卸载或替换 page 前保存 scroll offset、managed inset 状态和必要 appearance 状态。
4. `reloadData` 清理旧 page state、旧 fallback host content、旧 offset snapshot 和旧 Tabman/Pageboy 状态。
...
9. 记录 fallback child add/remove、cache window 更新、offset snapshot 保存/恢复、重复 view controller 降级日志。

验收：

1. fallback child add/remove containment 单测通过。
2. Tabman 驱动的 child appearance lifecycle 顺序测试通过。
...
设计与计划基线：`2026-07-13-v0-5-scroll-coordination-design.md`、`2026-07-13-boundary-bounce-ownership-design.md`、`2026-07-14-container-top-inset-fixed-header-presentation-design.md` 及对应 plans。2026-07-14 主容器真实 top inset、逻辑 container offset 和固定高度 Header presentation 已在生产 HEAD `424a0a3` 完成 TDD、最终全量验收与 fresh-pass：Framework 322/322、Example 41/41、generic Simulator build，均为 0 fail/skip/error/warning/analyzer warning；复审终态 Critical 0、Important 0、Minor 0，v0.5 Task 7/v0.6 恢复 Ready，并开放 v0.7。ScrollCoordinator 只能只读 Store 已提交的 current child/scroll target（empty 为 nil），并在 matching reload/selection complete/cancel terminal 后重新绑定；任何时刻都不得设置业务 child 的 `UIScrollView.delegate`，也不得设置 container/child 内建 pan delegate、缓存 Host/adapter/provider、读取 provider pending、复制 page identity/cache/generation 职责，或绕过空态猜测 current owner。
...
9. 记录 Header 完全展开、Header 完全折叠、child top boundary，以及 scroll/overscroll owner、handoff、boundary phase 的状态变化；guarded contentOffset correction 热路径不逐帧记录 apply/skip，只允许异常或受控采样。
10. 当前 container 与当前 child 允许受限纵向 simultaneous recognition，支持同一 pan 的剩余 delta 连续交接。
11. container 未完全折叠时当前 child 必须位于顶部；child 离开顶部时 container 必须完全折叠。
12. simultaneous recognition 只通过自有 container `UIScrollView` 子类放行当前 committed child pair；不得设置 container 或 child 的内建 pan delegate，child scroll delegate 保持原 owner。
13. stable range 与 native boundary pass-through 分离；顶部 mode 与底部 owner 路由以 `2026-07-13-boundary-bounce-ownership-design.md` 为准，不建立第二套纵向 owner。
...
6. 向上和向下 handoff 不丢失同一 pan 的剩余 delta。
7. Header 折叠热路径不改变 Pageboy child bounds。
8. UIKit 集成测试证明 child scroll/pan delegate 未被替换，binding resource 可同步释放。
9. Example UI test 使用真实连续 drag 覆盖双向 handoff、短内容、plain direct page、切页和真实顶部/底部 presentation。

### v0.6：顶部 Overscroll 事件处理版

目标：实现 none、container、child 三种顶部 overscroll handling 模式。

依赖约束：只消费 v0.5 已绑定的 committed current/empty owner；provider pending page 不参与 overscroll owner 判断。
...
5. `none` 模式把顶部收敛在 stable boundary，不保留原生 overscroll owner。
6. `container` 模式下，Header 完全展开后的继续下拉由 verticalScrollView 处理。
7. `child` 模式下，Header 完全展开后的继续下拉只由当前真实 child scroll view 处理；nil scroll target 不创建替代 owner且不回退。
...
设计基线：`2026-07-15-v0-7-interaction-selection-momentum-design.md`。依赖约束：PagingHost 建立唯一 active/latest pending selection request，复用 matching terminal 与 acknowledgement；Adapter 只执行 Tabman/Pageboy 并观察 paging surface，InteractionCoordinator 只做跨域仲裁，Scroll/Overscroll 保持唯一 offset/policy owner；不得创建第二套 generation owner。
...
9. 系统返回手势与横向分页手势有明确优先级，第一页 leading-edge 返回手势不被吞掉；child 横向 content scroll 必须先通过真实 UIKit 可行性验收，不能仅凭 failure relation recorder 声明能力。
10. 记录 interaction state begin、重要 update 边界、finish、cancel、非法 transition 忽略日志。
11. 捕获 container/current child pan velocity，并在 handoff boundary 通过 ScrollCoordinator 合成双向剩余惯性；不得设置业务 child delegate/pan delegate 或修改 bounce、`isScrollEnabled`。
...
7. container-to-child 与 child-to-container 真实 fling UI 测试通过，边界无停顿、反跳或双 owner。
8. 业务横向 child 若无法在既有所有权约束下安全实现，必须撤回候选生产关系并在 architecture/README 明确限制，不以私有 API、delegate 接管或手势 reset 掩盖。

完成状态：v0.7 生产代码 HEAD `07a3443` 已通过 Framework 426/426、Example 60/60（16 单元 + 44 UI）和 generic Simulator build，均为 0 fail、0 skip、0 error、0 warning、0 analyzer warning。fresh-pass 终态 Critical 0、Important 0、Minor 0；v0.7 Ready，可进入 v0.8。

2026-07-16 follow-up：横向-only 页面纵向目标修复生产代码 HEAD `984a009` 已关闭 Example 第五页默认 lookup，并提交 original Pageboy page + nil target；Public API、Framework ScrollCoordinator/Paging/containment 与业务手势所有权不变。Framework 426/426、Example 61/61（16 单元 + 45 UI）与 generic Simulator build 全部通过；fresh-pass 终态 Critical 0、Important 0、Minor 0，v0.7 继续为 Ready。

2026-07-16 Compositional Layout follow-up：生产代码 HEAD `db4b9bc` 已追加逐页交互分页策略和混合轴第六页。策略作为 reload metadata 由 PagingHost 原子提交，Adapter 只执行 Pageboy 自有 `isScrollEnabled`；Example index 4、index 5 为 `false`，业务横向内容到边界后不向 Pageboy 接力，bar/API 保持可用。Framework 439/439、Example 70/70（19 单元 + 51 UI）与 generic Simulator build 全部通过，0 fail、0 skip、0 error/warning/analyzer warning；运行时问题关键字零命中，fresh-pass 终态 Critical 0、Important 0、Minor 0，v0.7 继续为 Ready。

### v0.8：状态栏点击与尺寸变化版

目标：补齐系统级 UIKit 行为。

依赖约束：scrollsToTop 与尺寸恢复只消费 committed current/empty owner；provider pending page 不能成为系统 owner。
...
依赖约束：accessibility、Dynamic Type 与 RTL 的 current page/selection 语义只读取 committed visible state，不读取 provider pending。
...
6. 在 Swift 6.2+ 工具链下修复 Swift 6 language mode 并发警告。
...
5. Paging adapter 将 title 和 child view controller 交给 Tabman/Pageboy，由 Tabman/Pageboy 执行横向 page containment，并把 page change 事件收敛回 AnchorPager。
6. Page state store 管理 page identity、缓存窗口、offset snapshot 和 committed scroll identity；plain page 保持 original controller + nil scroll target。
7. ScrollCoordinator 在 v0.5 起只读 Store committed current/empty 入口，协调 verticalScrollView 与 terminal 后重新绑定的当前 child scroll view。
8. OverscrollCoordinator 在 v0.6 起处理顶部 overscroll owner。
9. InteractionCoordinator 在 v0.7 起统一处理跨域交互状态和优先级；PagingHost 统一 selection request，Adapter 标准化 Pageboy terminal，ViewController/Store 继续提交真实 current/public selection。
...
8. AnchorPager 自有 containment 调用失败不被静默吞掉，Debug 下触发 assertion，Release 中尽量回到空页或 fallback host 状态；横向 page containment 失败优先在 Tabman/Pageboy adapter 边界降级，不扩大 public API。
9. Pageboy 依赖升级必须重审 empty teardown、request provenance/terminal acknowledgement、appearance、provider activation 和 bar settlement，不能只以 resolve/build 通过作为兼容结论。
...
4. v0.4 覆盖 fallback containment、Tabman 驱动的 appearance lifecycle、reload、缓存和释放。
...
10. v1.0 使用 Swift 6.2+ 工具链运行完整验证命令并修复 Swift 6 language mode 并发警告。
...
1. 完整验证命令通过。
2. Swift 6.2+ 工具链下的 Swift 6 language mode 并发警告清理完毕。
3. 资源生命周期检查通过。
