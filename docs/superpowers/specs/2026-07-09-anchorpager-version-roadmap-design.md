...
启动门禁：已满足；v0.4 最终独立复审没有 Critical/Important/Minor 未解决项。v0.5 只能只读 Store 已提交的 current child/scroll target（empty 为 nil），并在 matching reload/selection terminal 后重新绑定；不得缓存 Host/adapter/provider、读取 provider pending、复制 page identity/cache/generation 职责，也不得绕过空态猜测 current owner。
