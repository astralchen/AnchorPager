...
8. `AnchorPagerBarConfiguration.height` 改为 optional，默认 nil 使用 Tabman 自适应高度，显式值约束实际 bar。
9. Tabman adapter 高度固定为 Header 完全折叠时的 viewport 高度，Header 折叠热路径只移动 adapter top。
10. 通过 Tabman public `barInsets.top` 获取 child 局部 bar obstruction，不把 Header 或容器顶部遮挡写入 child top inset。
11. 区分 managed inset 和外部追加 inset，不覆盖调用方原有 contentInset。
12. AnchorPager 接管的 child scroll view 设置 `contentInsetAdjustmentBehavior = .never`，ownership 结束时恢复原始行为。
13. 使用弱引用 ownership record，重复目标跳过写入并在释放时只移除最后一次 managed 部分。
14. 记录显式 scroll view 命中、默认 lookup 命中、fallback host、barInsets 变化、managed inset 写入、跳过和归还日志。
...
7. optional bar height、自适应 barInsets、ownership 归还和 fallback host inset 测试通过。
8. 示例工程真实列表页和 fallback 页 UI test 通过。
