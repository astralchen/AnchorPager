...
5. `Children`：page state store、缓存窗口、offset snapshot、fallback scroll host。
...
依赖约束：扩展 PagingHost 已有的单一 request/selection transaction，复用 request provenance、matching terminal 与 acknowledgement；不得创建第二套 generation owner。
...
6. Page state store 管理 page identity、缓存窗口、offset snapshot 和 fallback scroll host。
