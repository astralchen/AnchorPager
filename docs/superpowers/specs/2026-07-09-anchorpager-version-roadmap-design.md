...
7. 所有 UIKit 状态更新、public API、data source、delegate 和 coordinator 操作保持 MainActor 语义。
...
2. `Core`：主控制器内部状态、reload 流程、选择提交规则、公共 coordinator 汇聚点。
...
9. 建立 `docs/architecture.md`、`README.md` 和最小示例工程。
10. 建立核心单测目录，覆盖 public API 语义、scroll lookup、基础 Header 测量和基础 selectedIndex 行为。
...
10. Delegate 收到 didSelect、Header collapse progress 和 layout context 更新。
...
1. v0.1 覆盖 public API skeleton、基础分页、scroll lookup、基础 Header 和 selectedIndex。
