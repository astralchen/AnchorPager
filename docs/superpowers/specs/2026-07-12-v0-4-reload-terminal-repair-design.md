...
Pageboy 自身的公开重配置入口拆除内部 `UIPageViewController`，确认旧业务页已退出第三方 containment 后，再标准
移除旧 adapter、自身进入空状态并发送 `.empty` terminal。下一次非空 reload 创建新 adapter。
...
public `deletePage` 的零页路径会安装空 `UIViewController`，等同 sentinel，不能采用。源码核验确认 public
`interPageSpacing` 的 `didSet` 会让 Pageboy 自己执行 `reconfigurePageViewController()`：旧内部
`UIPageViewController` 由 Pageboy 标准移除，新实例在 count 为零时保持空内容。

因此 adapter 提供唯一 internal `prepareForRemoval()` 兼容入口：

1. 先把 adapter 的 count、titles、pending selection 状态收敛为空；
2. 若 view 已加载，执行 `interPageSpacing = interPageSpacing`，只调用 Pageboy public API，不读取或操作其
   internal `pageViewController`；
3. Pageboy 自己重建内部容器，旧业务页由 UIKit/Pageboy 结束 containment 和 appearance；
4. 方法返回后，PagingHost 才能移除 adapter 并发送 `.empty`。

该同值赋值依赖 Pageboy 5.0.2 的实现行为，而不是其文档化清空语义，必须视为受控兼容 shim：只能存在于
`Sources/AnchorPager/Paging/`，必须由真实 containment/appearance 测试锁定。任何 Pageboy 升级都必须先重新
审查 `interPageSpacing.didSet`、`reconfigurePageViewController()` 与零页 `reloadData()` 源码并运行这些测试；
若上游提供正式 clear/detach API，应立即替换 shim。不得把这个机制泄漏到 public API、Store 或主控制器。
...
3. adapter 先把 Pageboy data source 状态收敛为空，再通过受控 public reconfiguration shim 让 Pageboy 自己拆除
   旧内部 `UIPageViewController` 和业务页；
4. PagingHost 对已清空 adapter 调用 `willMove(toParent: nil)`；
5. 移除 adapter view并调用 `removeFromParent()`；
6. host 清空 adapter 引用并报告 barInsets `.zero`；
7. host 发送 `.empty` terminal；
8. ViewController commit pending generation；
9. Store 归还旧 ownership、清理旧 fallback 和 snapshot；
10. public 空页状态与实际可见层级同时成立。
...
3. adapter `prepareForRemoval()`：旧 scroll/fallback page 的 parent 和 view superview 同步清空，不发送
   page/selection terminal，正常非空 reload 与用户/程序化 selection 不受影响。
