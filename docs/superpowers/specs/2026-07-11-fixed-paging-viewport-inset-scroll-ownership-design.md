...
**日期：** 2026-07-11
**状态：** v0.3 已实现；v0.4 generation atomicity Task 1–3、完整验收与最终独立复审已完成；v0.5 专用设计草案待用户审阅，实施计划与实现尚未开始
**适用版本：** v0.3、v0.4、v0.5
...
7. Header 尚未完全折叠时，adapter 固定高度造成的 viewport 底部溢出也属于 child 坐标系内的
   本地底部遮挡，不能只使用 AnchorPager 根视图的 safe-area bottom。
...
6. adapter 当前底端到 AnchorPager 安全区底端之间的 child 本地底部遮挡。
...
设 AnchorPager 安全区可见底端为：

```text
safeVisibleMaxY = bounds.maxY - bottomObstruction
```

固定高度 adapter 在 Header 展开时会超出 viewport，因此 child 本地底部遮挡必须从实际 adapter
frame 推导：

```text
localBottomObstruction = max(0, pagingFrame.maxY - safeVisibleMaxY)
                       = bottomObstruction + collapsibleDistance - collapseOffset
```

Header 展开时该值包含“尚未折叠距离 + 根视图底部遮挡”；Header 完全折叠时自然收敛为根视图
bottom obstruction。不得把根视图 `safeAreaInsets.bottom` 直接当成 child 局部值，也不得通过改变
adapter height 修正这一差异。
...
container 折叠会改变 `localBottomObstruction`，因此滚动布局热路径允许只在该值实际变化时幂等更新
managed bottom。更新必须继续保留 `childDistanceFromTop`，不得改变 adapter/Pageboy child bounds，
不得逐帧输出日志。v0.5 中 container 尚未完全折叠时 child 被 owner 不变量锁定在顶部，动态 bottom
更新不会引入第二个纵向 owner；container 完全折叠、child 获得滚动所有权后，bottom target 已稳定为
根视图底部遮挡。v0.4 的 page offset snapshot 继续只保存 `childDistanceFromTop`，不保存派生的 bottom。
...
13. Header 展开、部分折叠和完全折叠时，managed bottom 分别等于 adapter 当前底端到安全区底端的
    局部距离；滚到最大 child offset 后最后内容和 indicator 均不进入底部遮挡区。
14. container 折叠更新 managed bottom 时保持 child distance-from-top，且 Pageboy child bounds 不变。
