...
- 设计提交 `6fe7e00`，实施计划提交 `afaefce`，生产源码与测试提交 `1f7e3f4`。
- 在 detached 临时 worktree 中恢复旧 `top ==` / `bottom <=` 约束后，真实 UI 用例按预期 RED：`headerContentTopDeltaMax = 116 pt`，失败结果包为 `/private/tmp/AnchorPagerExampleHeaderREDData/Logs/Test/Test-AnchorPagerExample-2026.07.15_11-12-47-+0800.xcresult`。同进程测试在旧约束下仍通过，确认真实多帧 safe-area 传播必须由 XCUITest 累计显示帧门禁覆盖。
- 主工作区保留正式 `top >=` / `bottom ==` 约束；Example 单元 target 与 `testPlainContainerTopBounceIsVisible`、`testHeaderContentKeepsTwentyPointTopSafeAreaPaddingWhenSwitchingBehaviors` 两条聚焦 UI 均 GREEN。
- Apple Swift 6.3.3 / Xcode 26.6、iPhone 17 Pro / iOS 26.5 下，`swift package resolve` 通过；Framework 322/322、Example 41/41（11 单元 + 30 UI）均为 0 fail、0 skip，generic Simulator build 成功。
- Framework、Example、build 结果包分别为 `/private/tmp/AnchorPagerExampleHeaderBounceFramework-20260715-1120.xcresult`、`/private/tmp/AnchorPagerExampleHeaderBounceExample-20260715-1121.xcresult`、`/private/tmp/AnchorPagerExampleHeaderBounceBuild-20260715-1128.xcresult`；三者均为 0 error、0 warning、0 analyzer warning。
- UIKit `LayoutConstraints` 查询无冲突；Public Tabman/Pageboy 扫描零命中，delegate/bounce 扫描只命中 AnchorPager 自有 `verticalScrollView` 的既有配置。
- fresh-pass 覆盖 `afaefce...1f7e3f4`，确认 Example probe 只读本地 frame，framework/Public/containment/offset writer/inset/gesture/bounce/logging owner 均未改变；结论为 Critical 0、Important 0、Minor 0。
