...
**适用范围：** v0.7 interaction state、快速选择请求、Tabman bar 点击、Pageboy 横向 pan、纵向跨 owner 惯性、reload/layout/尺寸仲裁和系统返回/业务横向手势优先级
...
GesturePriorityCoordinator 只管理 AnchorPager 自有的内部 recognizer 关系：

1. 弱持有 Pageboy paging pan、navigation interactive-pop gesture 和 committed current scroll target pan。
...
4. committed current scroll target 在当前 geometry 下具备横向滚动范围时，Pageboy paging pan 对该业务 pan 建立失败依赖，使命中业务横向内容的手势优先。
5. UIKit 没有公开的 failure-requirement removal API；旧页面离开实际命中层级后其 recognizer 不再参与新触摸。若 Pageboy paging surface identity 改变，则只为新 paging pan 重新建立当前关系。

该 coordinator 不遍历或接管任意业务手势，只消费 Store 已提交的 current scroll identity；非 current、provider pending 和 plain page nil target 不参与。
...
API 与 bar transaction 必须同时收到 semantic terminal 和 completion acknowledgement 才能释放 active 并推进 reload/latest pending。interactive transaction 只等待 did-select/did-cancel。

semantic did-select 到达时立即提交真实 current page；completion 只控制 transaction readiness，不重复发送 public didSelect。
...
5. `B` 的 completion acknowledgement 到达后释放 active；若 `D` 仍有效，直接启动 `B -> D`。
6. `B` 的旧 completion 只能 acknowledgement `B` 的 identifier，不能清除或覆盖 `D`。
7. 非相邻 `B -> D` 使用 Pageboy 单次 source/target transition，不逐页执行中间 index。
...
3. matching terminal + acknowledgement 完成后回 `idle`，然后 drain latest pending/reload/layout。
...
1. 等待已经交给 Pageboy 的 active selection matching terminal/acknowledgement。
...
3. committed current scroll target 满足 `contentSize.width + adjustedContentInset.left + adjustedContentInset.right > bounds.width + epsilon` 时，建立 `pagingPan.require(toFail: childPan)`；业务横向 content pan 在其命中区域优先，不修改业务 pan delegate、scroll delegate 或 `isScrollEnabled`。
4. UIKit public failure relation 不能安全表达“只在业务横向 scroll 到边界时动态交接”，v0.7 不新增业务 API模拟该语义。
5. container/current child 的纵向 simultaneous pair 继续只由 `AnchorPagerContainerScrollView` 放行；Pageboy pan 和无关 pan 不加入该 pair。
6. 真实优先级必须用导航栈、第一页、非第一页、全屏横向 child 和普通纵向 child 的 UIKit/UI 测试固定，不能只用直接调用 delegate 方法代替。
...
3. active completion 不清除 newer pending。
4. active target 完成后真实中间页提交，再直接启动 latest target。
5. active 期间请求 committed source index 能排队返回 source。
6. API、bar 使用相同 identifier/admission；Adapter bar override 不调用 Tabman 默认滚动路径。
7. interactive did/cancel、missing will recovery、duplicate/stale/out-of-order callback。
8. selection active + latest reload、reload pending + selection、teardown structural cancel。
9. Pageboy 真实 Adapter/Host 集成测试，不只使用 fake adapter 返回值。
...
5. 业务横向 scroll 命中区域优先级及页面其他区域 paging。
...
4. 系统返回、Pageboy、业务横向/纵向手势优先级已通过真实导航栈测试。
