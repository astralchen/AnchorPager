...
**验收记录：** 2026-07-12 generation atomicity 修复后使用 Swift 6.2.4 通过 AnchorPager 193 项框架测试、
示例工程 generic simulator build、示例工程 16 项 UI 测试和 5 项示例单元测试，均为 0 fail、0 skip；
resolve / Framework / Example build / Example test 墙钟分别为 1.34 / 53.81 / 15.71 / 277.97 秒；warning 仅为
Pageboy/Tabman `PrivacyInfo.xcprivacy` unhandled resource 提示。最终独立复审前不据此宣称 v0.4 Ready。
...
- staged public metadata，并在 Host matching terminal 提交 generation 与可见状态；
...
跨代状态拆为共享 live identity payload 与每代独立 state。payload 只弱引用 original/actual/scroll/fallback identity；单个 generation state 至少保存：
...
- Store dictionary 可以长期保留轻量 generation state 和 offset snapshot，但 payload 弱引用不得阻止页面释放；
...
相邻 generation 可共享仍存活的 live identity payload，但必须新建 generation state，不共享 retention reasons、
strong lease、ownership lease 或可变 snapshot。同一 index 迁移复制 committed distance；移动到不同 index 时新 state
distance 初始化为 `0`。pending reconcile/cancel 只能改变 pending state，terminal commit 前旧 committed fallback
containment、offset 和 ownership 保持不变；commit/cancel/releaseAll 在释放最后 strong lease 前强捕获 CleanupPlan，
再显式清理 unique fallback/scroll 资源。
...
每次完成 metadata 采集都会形成带递增 request identifier 的 staged snapshot。已有 committed visible generation 时收敛顺序如下：

1. 保持当前 public/Header、visible committed generation 和 ownership 不变，Host enqueue latest request。
2. matching `willPerform` 到达时才 begin provider generation；Pageboy provider 读取 pending。
3. active request 内的 bar inset 只 staged，不写旧 committed scroll。
4. matching page/empty terminal 到达后，Store commit provider generation。
5. ViewController 一次发布 public metadata/Header、terminal index 与 request-scoped bar inset，并返回 acknowledgement。
6. Store 以强 CleanupPlan 收敛旧 generation unique fallback/scroll 与 ownership。
7. Host 仅在 ack 为 true 时更新 committed bar baseline，并推进 latest pending。

首次 view 未加载且没有 committed visible generation 时，可预发布初始 metadata并幂等激活 provider；不加载 paging view，view load 后仍以同一 request terminal 正式提交。所有路径均不使用异步延迟或 timer 猜测 Pageboy 完成。
...
v0.5 的 ScrollCoordinator 只读 PageStateStore committed current page/scroll target（empty 为 nil），并在 matching
reload/selection terminal 后重新绑定；不得缓存 Host/adapter/provider、读取 provider pending 或接管页面身份/cache/generation。
v0.6/v0.8 只消费 committed current/empty owner；v0.7 扩展 Host 现有单一 request/selection transaction，不建立第二套
generation owner；v0.9 accessibility/RTL 不读取 provider pending。这样 page policy、纵向 owner 和交互仲裁保持单向依赖。
