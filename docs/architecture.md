...
可见状态下的程序化切页采用确认后提交语义：`AnchorPagerViewController` 只在内部 adapter 收到 Pageboy/Tabman 的完成回调后更新 public `selectedIndex` 并通知 delegate；取消或回弹不会提前提交。当前 v0.1–v0.6 在 Pageboy 忙碌时拒绝新程序化请求，Adapter 保留已接受的上一笔 pending selection。v0.7 已确认改为 Host 管理一笔 active + 一笔 latest pending，并以 matching identifier 防止旧 completion 清除新意图；真实中间 terminal 仍提交 Store/public selection，完整设计见 `docs/superpowers/specs/2026-07-15-v0-7-interaction-selection-momentum-design.md`，未实施前不得把该目标语义描述为当前能力。
...
`AnchorPagerPagingHostViewController` 是 Header 下方布局约束的长期 owner。当前 v0.4–v0.6 实现管理 reload request 串行、adapter containment 和事件转发，并通过 Adapter readiness 等待 selection terminal；实际 programmatic pending/completion 仍暂存在 Adapter。每笔 reload request 都带 internal identifier；Host 同时最多有一个 active request 和一个 latest pending request，只有 matching adapter callback 才能形成 `.page(index:)`/`.empty` terminal。selection 活跃时，pending reload 由 adapter did/cancel 语义 terminal（程序化路径还要等 Pageboy completion）推进，不使用 timer 或主队列 delay猜测第三方完成时机。terminal 通知期间的重入和旧 request 迟到 callback 不能结束新 active request。

v0.7 已确认把 selection request admission 正式收口到 Host：一笔 active + 一笔 latest pending、Host identifier、Adapter 第三方执行、ViewController/Store 真实 terminal 提交。Pageboy 5.0.2 的动画 completion 早于内部 `isScrollingAnimated` 清零，因此 Host 还必须等待 Adapter 通过 Pageboy 的 open `isUserInteractionEnabled` 覆写点发布 matching executor-ready，不能在 completion 调用栈内 drain latest selection。该目标契约尚未实施，详见 `docs/superpowers/specs/2026-07-15-v0-7-interaction-selection-momentum-design.md`。
