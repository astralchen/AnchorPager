...
本文档面向维护者，记录当前 page generation、固定分页 viewport、纵向 handoff、边界 bounce 与顶部 owner 路由的架构边界。v0.5/v0.6 实现已完成实现者自审和新鲜全量验收，仍待主代理比较 `be2d783...47abcd6` 执行独立复审；在该门禁完成前不标记 Ready。
...
2026-07-13 使用 Apple Swift 6.3.3、iPhone 17 Pro / iOS 26.5 新鲜验收：`swift package resolve` 通过；Framework 264 项、Example 9 项单元 + 27 项 UI 全部通过，0 fail、0 skip；Example generic Simulator build 成功。framework test、Example test、Example build 三份 xcresult 均为 0 error、0 warning、0 analyzer warning。
...
- v0.5/v0.6 主代理独立复审尚未完成；当前只有实现者自审与 Framework 264 项、Example 36 项、generic build 的新鲜通过证据，因此不标记 Ready
