...
本文档面向维护者，记录当前 page generation、固定分页 viewport、纵向 handoff、边界 bounce 与顶部 owner 路由的架构边界。v0.5/v0.6 初次独立复审的 3 个 Important 与再次整分支复审剩余的 1 个 Important、1 个 Minor 均已修复并完成新鲜全量验收；修复后的再次独立复审待执行，在该门禁完成前不标记 Ready。
...
2026-07-13 使用 Apple Swift 6.3.3、iPhone 17 Pro / iOS 26.5 对修复提交 `5b80893` 新鲜验收：`swift package resolve` 通过；Framework 276 项、Example 10 项单元 + 27 项 UI 全部通过，0 fail、0 skip；Example generic Simulator build 成功。framework test、Example test、Example build 三份 xcresult 均为 0 error、0 warning、0 analyzer warning。
...
- v0.5/v0.6 已完成当前复审问题修复，并有 Framework 276 项、Example 37 项、generic build 的新鲜通过证据；修复后的再次独立复审待执行，因此不标记 Ready
