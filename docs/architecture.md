...
后续“横向业务边缘下一次手势分页接力”设计已确认但尚未实现：目标是删除上述 Public Bool 与 generation-aware 策略链，不增加外部协议；Paging surface 将增加框架自有 route gate，只在手势起点沿实际命中视图祖先读取公开 `UIScrollView` 横向 range/offset。内容仍可消费时 gate 只阻止 Pageboy 并与业务 pan 同时识别；所有候选已在对应外边缘时 gate 失败，让 Pageboy 原生分页。当前手势中途到边缘不重新仲裁。该方案不得设置 Pageboy/业务内建 pan delegate、写业务 offset 或识别私有类名，且必须先通过普通嵌套 scroll 与原生 orthogonal 的真实 UIKit 硬门禁；门禁前当前生产静态策略仍是事实。详见 `docs/superpowers/specs/2026-07-16-next-gesture-horizontal-boundary-paging-design.md`。
...
- 已确认后续自动边界路由设计，但 production/TDD/真实 UI 尚未开始；在新硬门禁完成前不得把“下一次手势接力”描述为已交付能力
