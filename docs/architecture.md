...
本文档面向维护者，记录当前 page generation、固定分页 viewport、纵向 handoff、边界 bounce 与顶部 owner 路由的架构边界。2026-07-14 plain bottom 页面/chrome 分层、Header identity cache、正式中立测量和真实内容附着前 bootstrap seed 均已完成历史全量验收。用户随后确认主容器真实 top inset 与固定高度 Header presentation 新契约；专项实现、全量验收和独立复审待完成，v0.5 Task 7 与 v0.6 Ready 门禁当前重新关闭。

## 待实施架构修订：Container Top Inset 与固定高度 Header

最新确认设计见 `docs/superpowers/specs/2026-07-14-container-top-inset-fixed-header-presentation-design.md`。当前生产代码仍使用 container top inset 为零、raw 稳定区间 `0...collapsibleDistance` 并在折叠热路径缩小 Header host；该实现只作为待替换的现状记录，不能作为后续实现契约。

目标架构保留 `contentInsetAdjustmentBehavior = .never`，但由 `AnchorPagerHeaderTopBehavior` 独立拥有主容器 top inset：inside 等于本地顶部安全区遮挡，extends 为零。所有纵向协调统一使用 `logicalOffset = rawOffset + containerTopInset`；固定 viewport 内新增 canonical content presentation surface，HeaderHost 保持完整高度，Header 与 PagingHost 通过该 surface 正常上移。container top 仍移动共享 viewport，plain bottom 仍只移动 Pageboy 页面 surface，真实 child bounce 仍由业务 child 表达。完成实现与验收前不得进入 v0.7。
...
2026-07-14 当前验收使用 Apple Swift 6.3.3、Xcode 26.6、iPhone 17 Pro / iOS 26.5：`swift package resolve` 通过；生产代码 HEAD `c37e829` 对应 Framework 293/293，Example 37/37（10 单元 + 27 UI），全部 0 fail、0 skip；generic Simulator build 成功。三份最终结果均为 0 error、0 warning、0 analyzer warning。
...
主容器使用两个互不反向约束的内部层：`scrollRangeView` 约束到 `contentLayoutGuide`，高度固定为 viewport 高度加纯内容可折叠距离，只负责定义 `contentSize`；`viewportView` 约束到 `frameLayoutGuide`，承载 Header host 和 paging adapter。Header/paging 的可见约束不会参与 scroll range 反算，当前 `contentOffset` 也不会改变 `contentSize`。container overflow 物理仍来自 `verticalScrollView`，但 presentation 分层：共享 chrome 的 `viewportTranslationY = topOverflow`，adapter 页面 surface 的 `pageSurfaceTranslationY = -plainBottomOverflow`，页面最终可见总位移为两者之和。顶部越界使 Header/bar/page 整体下移；plain bottom 越界只使 Pageboy 页面 surface 上移，Header/bar 保持 canonical。两类位移都由 UIKit 自身 bounce 动画驱动恢复，且不进入 Auto Layout、scroll range、managed inset、snapshot 或 generation。`verticalScrollView.delegate` 由内部私有 proxy 管理，调用方不得替换。
...
`AnchorPagerLayoutContext` 中的 frame 使用 `AnchorPagerViewController.view` 的本地实际可见坐标。LayoutEngine output 和 `lastLayoutOutput` 保持 canonical geometry；container top 时三个 frame 加入相同正向位移，plain bottom 时 Header/bar frame 保持 canonical、只有 content frame 加入负向页面位移。`scrollViewDidScroll` 复用最近一次有效纯内容测量，只更新 canonical output、分层 presentation、layout context 和 collapse progress；它不重新测量 Header、不修改 scroll range，也不输出逐帧普通日志。v0.5 已完成 committed current child 的连续双向 handoff、稳定区间 offset 分配与原生边界 owner 路由；v0.7 仅在当前回归重新验收后扩展跨 owner 惯性合成和完整 interaction state。
...
当前生产代码已实现固定分页 viewport、child inset ownership、按需 page state/cache window、reload generation、连续纵向 handoff、plain direct page、stable/native boundary 分离、三种顶部 owner 路由、plain bottom 分层 page presentation，以及 Header 正式 measurement 与内容附着前 bootstrap；这些路径的历史全量验收与 fresh-pass 复审均已完成。主容器真实 top inset 与固定高度 Header presentation 尚待实施和重新验收，因此 v0.5/v0.6 当前不处于 Ready。仍不包含：
