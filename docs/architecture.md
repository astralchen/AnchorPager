...
本文档面向维护者，记录当前 page generation、固定分页 viewport、纵向 handoff、边界 bounce 与顶部 owner 路由的架构边界。2026-07-13 的四轮复审问题均已修复并形成历史 Ready；2026-07-14 后续用户验收发现 plain bottom 共享 viewport presentation 会移动 bar，且 Header 首次 required zero-height 中立布局会触发内容约束冲突。分层 page presentation 与 bootstrap measurement 设计已确认，实施、全量验收和独立复审待完成，当前 v0.5 Task 7 与 v0.6 不标记 Ready。
...
2026-07-13 历史验收使用 Apple Swift 6.3.3、iPhone 17 Pro / iOS 26.5：`swift package resolve` 通过；生产代码 HEAD `128821f` 对应 Framework 283/283，Example 37/37（10 单元 + 27 UI），全部 0 fail、0 skip；generic Simulator build 成功。该证据早于 2026-07-14 新回归，不作为当前 Ready 证据。
...
负数或非有限测量会触发内部断言，并在 `layout` category 记录 `header.measure.invalid` 事件，运行时降级为 `0`。结构性布局会先清除 viewport/page presentation，把 Header host 临时放到顶部遮挡下方，并使用最近一次有效纯内容高度建立中立测量几何。2026-07-14 修订要求首次无缓存时先取得不发布状态的 compressed fitting bootstrap seed，再以该 seed 建立 required host height 和同步中立 layout；非空约束内容不得以 required `height == 0` 参与 layout。正式 fitting 才更新测量缓存和 canonical output，临时几何不更新 layout context、progress、range 或状态日志缓存。
...
当前已实现固定分页 viewport、child inset ownership、按需 page state/cache window、reload generation、连续纵向 handoff、plain direct page、stable/native boundary 分离和三种顶部 owner 路由；plain bottom 分层 page presentation 与 Header bootstrap measurement 已完成设计但尚未实施/重新验收，因此 v0.5/v0.6 当前不标记 Ready。仍不包含：
