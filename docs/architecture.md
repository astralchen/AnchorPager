...
v0.7 Task 2–3 已实现 selection request 的 Host 单调 identifier、active/latest admission、source 路由、matching transaction 和 Adapter 单 execution 边界；Tabman bar 请求只回到 Host，不再由 Adapter 旁路调用 Pageboy。Pageboy 5.0.2 的动画 completion 早于内部 `isScrollingAnimated` 清零，因此 Adapter 只在 completion 后通过 Pageboy 的 open `isUserInteractionEnabled` 覆写点发布一次 matching executor-ready；非动画 completion 同步 acknowledgement completion 与 ready。completion 缺少 semantic 时，Host 只按 matching Adapter current index 恢复 did-select，或在 `finished == false` 时恢复 cancel；旧 adapter、identifier、target 和乱序 ready 均不释放 active。selection/reload 交叉时丢弃旧 generation pending selection、结构性 cancel 和 reload 优先 drain 仍由 Task 4 收口，不能在动画 completion 调用栈内启动 latest selection。完整契约见 `docs/superpowers/specs/2026-07-15-v0-7-interaction-selection-momentum-design.md`。
