...
本文档面向维护者，记录当前 v0.4 Child 生命周期、缓存与 reload 代际原子性阶段的架构边界和已固定契约。Task 1–3 实现、聚焦复审及 Swift 6.2.4 完整验收已完成；最终独立复审仍是 v0.4 可合并及 v0.5 启动门禁。
...
- v0.4 Task 1–3 已完成 Host request/terminal 与 bar acknowledgement、Store payload/generation lease/CleanupPlan、ViewController staged public/provider/bar terminal 和 pre-load 路径；完整验收为框架 193 项、Example 5 项单元与 16 项 UI 测试全部通过，最终独立复审已清零 Critical/Important/Minor，v0.4 Ready 并开放 v0.5 入口
- v0.5 尚未实现；ScrollCoordinator 只能只读 Store committed current child/scroll target（empty 为 nil），在 reload/selection terminal 后重新绑定，不缓存 Host/adapter/provider、不读取 provider pending、不复制 page identity/cache/generation
