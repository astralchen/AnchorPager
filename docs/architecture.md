...
真实 simulator pan 证明，仅靠 KVO 在 child 产生负 offset 后归顶，业务 delegate 仍可能先观察到瞬态 child bounce。为闭合唯一顶部 bounce owner，committed child 位于顶部或 container pan 活跃期间，binding 临时关闭其 `bounces`；手势结束且 child 离开顶部后恢复绑定前值，rebind、empty 和 invalidate 也同步恢复。该租约不修改 child scroll delegate、pan delegate、`isScrollEnabled`、inset 或 page state；正式 overscroll 配置仍由 v0.6 在同一 binding 上替换。
...
- v0.5 实施中，Task 1–6 已完成；专用设计已根据 UIKit 实测与 JXPagingView 参考验证修订为 child delegate 保留、container scroll view 子类 simultaneous recognition、canonical distance、可恢复 child bounce 租约、临时 container bounce 和真实 pan UI 门禁。ScrollCoordinator 只能只读 Store committed current child/scroll target（empty 为 nil），在 reload/selection complete/cancel terminal 后重新绑定，不缓存 Host/adapter/provider、不读取 provider pending、不复制 page identity/cache/generation
