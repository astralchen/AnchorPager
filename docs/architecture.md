...
本文档面向维护者，记录当前 v0.4 Child 生命周期、缓存与 reload 代际原子性阶段的架构边界和已固定契约。Task 1–3 实现、聚焦复审、Swift 6.2.4 完整验收及最终独立复审均已完成；v0.4 Ready，v0.5 实施中，Task 1–6 已完成，Task 7 最终验收尚未开始。
...
v0.5 当前尚未实现正式 overscroll mode。真实模拟器进一步发现 canonical clamp 会抵消肉眼可见的 container bounce，且现有 UI test 只捕获瞬时负 offset、不能证明 presentation 位移。修订目标见 `docs/superpowers/specs/2026-07-13-boundary-bounce-ownership-design.md`：先分离 stable range 与 native boundary pass-through、补齐无滚动页双边界 presentation，再由 v0.6 OverscrollCoordinator 使用同一纵向 binding 按 `.none`、`.container`、`.child` 路由正式顶部 owner，不重复建立 handoff。

JXPagingView 源码验证确认，顶部行为应通过 simultaneous pan、稳定边界 offset gate 和原生 owner 路由表达，不应通过框架强制修改业务 child 的 bounce 能力。AnchorPager 因此会移除当前过渡实现中的 `bounces` 租约：Binding 只保留不占用 delegate 的 contentOffset/contentSize observation 与 pan target，永远不保存、修改或恢复业务 child 的 `bounces`、`alwaysBounceVertical`、scroll delegate、pan delegate 或 `isScrollEnabled`。`.child` 顶部及真实 child bottom 是否出现原生回弹取决于业务 scroll view 自身配置；短内容由业务方决定是否启用 `alwaysBounceVertical`。

由于 AnchorPager 不接管业务 delegate，也不修改业务 bounce 属性，UIKit 可能先向业务 delegate 发布瞬时非 owner 越界，然后内部 observation 才执行 guarded stable-boundary write。框架契约是非 owner 不形成持续可见 presentation、稳定位置正确且只有选定 owner 保留原生越界，不再承诺业务 delegate 从未观察到瞬时负 offset。
...
2026-07-13 无滚动页面修复已按 `docs/superpowers/specs/2026-07-13-plain-page-direct-containment-design.md` 完成：synthetic scroll wrapper 已删除，original page 直接由 Pageboy/UIKit containment；Store 保持 page 非 nil、scroll target 为 nil，不应用 managed inset、snapshot、child bounce 或 simultaneous pair。UIKit 与 Example 真实 pan 验证页面根 view 至少覆盖 AnchorPager/window 物理底边，v0.5 Task 7 已重新开放但尚未完成。
...
- v0.5 实施中，Task 1–6 已完成；专用设计已根据 UIKit 实测与 JXPagingView 源码验证修订为 child delegate/bounce 配置保留、container scroll view 子类 simultaneous recognition、canonical distance、stable/native boundary 分离和真实可见 presentation UI 门禁。ScrollCoordinator 只能只读 Store committed current child/scroll target（empty 为 nil），在 reload/selection complete/cancel terminal 后重新绑定，不缓存 Host/adapter/provider、不读取 provider pending、不复制 page identity/cache/generation
- v0.5 Task 7 因可见 bounce 验收缺口再次暂停；边界 pass-through、对称 container presentation 与 v0.6 顶部 owner 路由已经完成设计确认，但尚未实施，不得把瞬时 offset flag 视为可见 bounce 证据
