...
可见状态下的程序化切页采用确认后提交语义：`AnchorPagerViewController` 只在内部 adapter 收到 Pageboy/Tabman 的完成回调后更新 public `selectedIndex` 并通知 delegate；取消或回弹不会提前提交。v0.7 Task 2 已把 Adapter 收口为 identifier-aware 单 execution executor：API、bar 与 interactive source 的 request identifier 由 Host 分配，Adapter 的 will/did/cancel/completion/ready 回调必须携带 matching identifier，旧或 target 不匹配的 callback 不能清除当前 execution。Host 的一笔 active + 一笔 latest pending admission 和完整 drain 尚待 Task 3 实现；真实中间 terminal 仍提交 Store/public selection，不能把 Adapter execution 当成第二套 public selection 状态。
...
v0.7 Task 2 已实现 selection request 的 Host 单调 identifier、source 路由和 Adapter 单 execution 边界；Tabman bar 请求只回到 Host，不再由 Adapter 旁路调用 Pageboy。Pageboy 5.0.2 的动画 completion 早于内部 `isScrollingAnimated` 清零，因此 Adapter 只在 completion 后通过 Pageboy 的 open `isUserInteractionEnabled` 覆写点发布一次 matching executor-ready；非动画 completion 同步发布 completion 与 ready，teardown 会先清除 execution/ready context。当前 Host 只完成了这套协议的最小签名桥接，一笔 active + 一笔 latest pending admission、matching terminal 和 reload 优先 drain 仍由后续 Task 3–4 收口，不能在 completion 调用栈内直接启动 latest selection。完整契约见 `docs/superpowers/specs/2026-07-15-v0-7-interaction-selection-momentum-design.md`。
