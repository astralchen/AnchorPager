...
- 自动 route gate 已完成真实 UIKit 停止门禁并以 0/2 失败：`pagingPan -> routeGate` 会阻断普通业务横向 scroll 与原生 orthogonal 的 interior 消费；Task 1/2 只是不装配的内部实验基础，当前静态 Bool 策略继续作为生产事实，Task 4 及完整验收均为 blocked/not started
