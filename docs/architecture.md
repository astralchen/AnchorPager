...
本文档面向维护者，记录当前 page generation、固定分页 viewport、纵向 handoff、边界 bounce 与顶部 owner 路由的架构边界。v0.5/v0.6 初次独立复审发现的 3 个 Important 已修复并完成新鲜全量验收；再次独立复审待执行，在该门禁完成前不标记 Ready。
...
2026-07-13 使用 Apple Swift 6.3.3、iPhone 17 Pro / iOS 26.5 对修复提交 `f81ca1e` 新鲜验收：`swift package resolve` 通过；Framework 271 项、Example 10 项单元 + 27 项 UI 全部通过，0 fail、0 skip；Example generic Simulator build 成功。framework test、Example test、Example build 三份 xcresult 均为 0 error、0 warning、0 analyzer warning。
...
active owner 只有在实际 overflow 超过可见阈值后才成为已呈现 owner。若业务 child 的原生 bounce 配置让 owner 已创建但始终未呈现，而同一 pan 反向回到 stable range，ScrollCoordinator 会同步结束该未呈现 owner并立即重新应用 resolver，避免把反向 delta 丢到 pan end；已呈现 owner 仍只在实际 overflow 回稳后结束。child KVO 观察到顶部负 offset 时，只有 container 已展开到 epsilon 内才允许进入 top owner；Header 部分折叠时始终保持 canonical container offset、把 child 钉回顶部且不创建 owner，这一门禁不依赖 container/child 回调顺序。
...
- v0.5/v0.6 初次独立复审问题已修复并有 Framework 271 项、Example 37 项、generic build 的新鲜通过证据；再次独立复审待执行，因此不标记 Ready
