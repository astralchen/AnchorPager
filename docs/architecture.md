...
本文档面向维护者，记录当前 page generation、固定分页 viewport、纵向 handoff、边界 bounce 与顶部 owner 路由的架构边界。2026-07-14 plain bottom 页面/chrome 分层、Header identity cache、正式中立测量和真实内容附着前 bootstrap seed 均已完成全量验收；v0.5 Task 7 与 v0.6 当前为 Ready。
...
当前已实现固定分页 viewport、child inset ownership、按需 page state/cache window、reload generation、连续纵向 handoff、plain direct page、stable/native boundary 分离、三种顶部 owner 路由、plain bottom 分层 page presentation，以及 Header 正式 measurement 与内容附着前 bootstrap。最新全量验收与 fresh-pass 复审均已完成，v0.5/v0.6 当前为 Ready。仍不包含：
