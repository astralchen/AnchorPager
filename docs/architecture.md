...
2026-07-13 真实视图层级检查发现，当前 `AnchorPagerPageScrollHostViewController` 把无 scroll 业务 child 的显示高度错误地绑定到 managed inset 后的滚动内容高度，导致根 view 底边不能到达物理屏幕底部。已确认的目标架构见 `docs/superpowers/specs/2026-07-13-plain-page-direct-containment-design.md`：移除 synthetic fallback scroll host，无滚动 original page 直接由 Pageboy/UIKit containment，Store 保持 page 非 nil、scroll target 为 nil，且不应用 managed inset、snapshot 或 child bounce。该修复完成前 v0.5 Task 7 不得开始。
